...
from m59_ui_dialogs import (
    ...
    M59StandaloneDockWindow, M59ComposeArticleDialog, M59ComposeMailDialog,
    M59FirstTimeSyncDialog
)
...
class M59CompanionApp(QMainWindow):
    def __init__(self):
        ...
        # News Globe & In-Game Mail Engine
        self.news_db = NewsGlobeDatabase()
        self.news_engine = NewsSyncEngine(db=self.news_db, gps_manager=self.gps_manager, packet_injector=self.inject_game_packet)
        self.news_engine.register_update_callback(self.on_news_engine_event)
        self.selected_news_nid = NID_ANNOUNCEMENTS
        self.selected_article_data = None
        self.selected_mail_data = None
    ...
    def setup_ui(self):
        ...
        # Section 9: Globe News & In-Game Mail Page
        self.page_news_globe = self.build_news_globe_page()
        self.stacked_widget.addWidget(self.page_news_globe)
    ...
    # ==================================================================
    # SECTION 8: GLOBE NEWS & IN-GAME MAIL CLIENT (REAL-WORLD ENGINE)
    # ==================================================================
    def on_news_engine_event(self, event_type, data):
        ...
        if event_type == "room_changed":
            ...
            self.update_news_globe_location_ui(room_name, globes)
            if globes:
                primary_nid = globes[0]["nid"]
                self.focus_newsgroup_tab(primary_nid)

        elif event_type == "prompt_first_time_sync":
            nid = data.get("nid")
            ng_name = data.get("newsgroup_name")
            total_count = data.get("total_count", 0)
            dlg = M59FirstTimeSyncDialog(ng_name, nid, total_count, parent=self)
            if dlg.exec() == QDialog.Accepted:
                self.news_engine.confirm_first_time_sync(nid)

        elif event_type in ("articles_indexed", "article_body_received"):
            self.refresh_news_globe_ui()

        elif event_type == "reply_notification":
            ng_name = data.get("newsgroup", "News Globe")
            poster = data.get("poster", "Someone")
            title = data.get("title", "")
            self.show_toast_notification(f"💬 Reply in {ng_name}: {poster} posted '{title}'", category="info")
            self.refresh_news_globe_ui()

    def focus_newsgroup_tab(self, nid):
        """Automatically focuses the Newsgroup tab matching the active room globe."""
        if not hasattr(self, 'news_tabs'):
            return
        for idx in range(self.news_tabs.count()):
            widget = self.news_tabs.widget(idx)
            if widget and widget.property("nid") == nid:
                self.news_tabs.setCurrentIndex(idx)
                break

    def build_news_globe_page(self):
        page = QWidget()
        page_layout = QVBoxLayout(page)
        page_layout.setContentsMargins(18, 18, 18, 18)
        page_layout.setSpacing(14)

        # 1. Header & Proximity Status Bar Card
        hdr_card = QFrame()
        hdr_card.setProperty("class", "WebCard")
        ...
        title_box = QVBoxLayout()
        t_lbl = QLabel("🌐 Globe News Reader & In-Game Mail")
        t_lbl.setStyleSheet("font-size: 16px; font-weight: 800; color: #f8fafc;")
        s_lbl = QLabel("Newsgroup Reader with location-based auto-sync, reply notifications, and mail client.")
        s_lbl.setStyleSheet("font-size: 12px; color: #64748b;")
        title_box.addWidget(t_lbl)
        title_box.addWidget(s_lbl)
        ...
        self.news_sync_btn = QPushButton("🔄 Sync Globe")
        self.news_sync_btn.setProperty("class", "WebBtnPrimary")
        self.news_sync_btn.setToolTip("Request latest article index for globes in your current room.")
        self.news_sync_btn.clicked.connect(self.trigger_globe_sync)
        top_row.addWidget(self.news_sync_btn)

        self.news_compose_btn = QPushButton("✏️ Post Article")
        self.news_compose_btn.setProperty("class", "WebBtnSecondary")
        self.news_compose_btn.setToolTip("Post a new article to a news globe.")
        self.news_compose_btn.clicked.connect(lambda: self.open_compose_article())
        top_row.addWidget(self.news_compose_btn)

        self.news_mail_compose_btn = QPushButton("✉️ New Mail")
        self.news_mail_compose_btn.setProperty("class", "WebBtnSecondary")
        self.news_mail_compose_btn.setToolTip("Send private in-game mail to another player.")
        self.news_mail_compose_btn.clicked.connect(lambda: self.open_compose_mail())
        top_row.addWidget(self.news_mail_compose_btn)

        self.news_mail_check_btn = QPushButton("📥 Check Mail")
        self.news_mail_check_btn.setProperty("class", "WebBtnSecondary")
        self.news_mail_check_btn.setToolTip("Request in-game mail queue.")
        self.news_mail_check_btn.clicked.connect(self.check_in_game_mail)
        ...
        prox_bar = QFrame()
        prox_bar.setStyleSheet("background-color: #0f172a; border-radius: 6px; border: 1px solid #1e293b; padding: 6px 12px;")
        prox_layout = QHBoxLayout(prox_bar)
        prox_layout.setContentsMargins(4, 2, 4, 2)
        prox_layout.setSpacing(14)

        self.news_room_loc_lbl = QLabel("📍 Location: Unknown")
        self.news_room_loc_lbl.setStyleSheet("font-size: 11px; font-weight: 700; color: #f8fafc;")
        prox_layout.addWidget(self.news_room_loc_lbl)

        self.news_proximity_badge = QLabel("⚪ Checking Proximity...")
        self.news_proximity_badge.setStyleSheet("font-size: 11px; font-weight: 700; color: #94a3b8;")
        prox_layout.addWidget(self.news_proximity_badge, 1)

        self.news_db_count_lbl = QLabel("DB: 0 Articles | 0 Mail")
        self.news_db_count_lbl.setStyleSheet("font-size: 11px; color: #64748b;")
        prox_layout.addWidget(self.news_db_count_lbl)

        hc_layout.addWidget(prox_bar)
        page_layout.addWidget(hdr_card)

        # 2. Main Newsgroup Reader Tabs
        self.news_tabs = QTabWidget()
        self.news_tabs.setStyleSheet("""
            QTabWidget::pane {
                border: 1px solid #1e293b;
                border-radius: 8px;
        ...
        # Build Newsgroup Tabs
        self.newsgroup_tab_map = {}
        self.newsgroup_widgets = {}

        newsgroup_configs = [
            (NID_ANNOUNCEMENTS, "Designers' News", "Official announcements and server patches"),
            (NID_TOS_HALL, "General News", "Adventurer Hall community newsgroup across all towns"),
            (NID_ADVENTURE, "Tales of Adventure", "Bardic lore and adventure stories"),
            (NID_JUSTICAR, "Justicar & Court", "Barloque Court legal news and warrants"),
            (NID_GAME, "Game Updates", "Technical maintenance and patch notes"),
            (NID_EVENT_SCHEDULE, "Event Schedule", "Player tournaments and community events")
        ]

        for nid, title, desc in newsgroup_configs:
            tab_widget = self.create_newsgroup_reader_tab(nid, title, desc)
            tab_widget.setProperty("nid", nid)
            self.newsgroup_widgets[nid] = tab_widget
            tab_idx = self.news_tabs.addTab(tab_widget, f"📰 {title}")
            self.newsgroup_tab_map[nid] = tab_idx

        # Add Mailbox Tab
        mail_tab_widget = self.create_mailbox_tab()
        mail_tab_widget.setProperty("nid", -1)
        self.news_tabs.addTab(mail_tab_widget, "📬 In-Game Mailbox")

        self.news_tabs.currentChanged.connect(self.on_news_tab_changed)

        page_layout.addWidget(self.news_tabs, 1)

        self.refresh_news_globe_ui()
        self.update_news_globe_location_ui(getattr(self, 'current_room_name', 'Unknown Location'))

        return page

    def create_newsgroup_reader_tab(self, nid, title, desc):
        """Creates a dedicated Newsgroup Reader View for a specific NID."""
        tab_widget = QWidget()
        layout = QVBoxLayout(tab_widget)
        layout.setContentsMargins(12, 12, 12, 12)
        layout.setSpacing(10)

        # Header bar with info & filter
        top_bar = QHBoxLayout()
        ...
        top_bar.addLayout(info_v, 1)

        search_input = QLineEdit()
        search_input.setPlaceholderText("🔍 Filter by subject or author...")
        search_input.setStyleSheet("background-color: #0f172a; border: 1px solid #1e293b; border-radius: 5px; padding: 6px 10px; color: #f8fafc; font-size: 11px;")
        search_input.textChanged.connect(lambda text, n=nid: self.filter_newsgroup_articles(n, text))
        top_bar.addWidget(search_input, 1)

        count_badge = QLabel("0 Articles")
        count_badge.setStyleSheet("font-size: 11px; font-weight: 700; color: #94a3b8; background-color: #0f172a; border: 1px solid #1e293b; border-radius: 4px; padding: 4px 8px;")
        top_bar.addWidget(count_badge)

        layout.addLayout(top_bar)

        # Splitter: Table on top, Reading pane on bottom
        v_splitter = QSplitter(Qt.Vertical)

        # Articles Table
        table = QTableWidget()
        table.setColumnCount(5)
        table.setHorizontalHeaderLabels(["#", "Status", "Date / Time", "Author", "Subject"])
        ...
        """)
        table.itemSelectionChanged.connect(lambda n=nid: self.on_newsgroup_article_selected(n))
        v_splitter.addWidget(table)

        # Reader Card
        reader_card = QFrame()
        reader_card.setStyleSheet("background-color: #0f172a; border-radius: 6px; border: 1px solid #1e293b; padding: 10px;")
        rc_layout = QVBoxLayout(reader_card)
        rc_layout.setContentsMargins(8, 8, 8, 8)
        rc_layout.setSpacing(8)

        # Reader Meta & Actions Row
        rh_row = QHBoxLayout()
        meta_v = QVBoxLayout()
        subj_lbl = QLabel("Select an article above to read")
        subj_lbl.setStyleSheet("font-size: 14px; font-weight: 800; color: #f8fafc;")
        meta_lbl = QLabel("--")
        meta_lbl.setStyleSheet("font-size: 11px; color: #64748b;")
        meta_v.addWidget(subj_lbl)
        meta_v.addWidget(meta_lbl)
        rh_row.addLayout(meta_v, 1)

        reply_btn = QPushButton("↩️ Reply")
        reply_btn.setProperty("class", "WebBtnSecondary")
        reply_btn.setEnabled(False)
        reply_btn.clicked.connect(lambda _, n=nid: self.reply_to_selected_article_for_nid(n))
        rh_row.addWidget(reply_btn)

        mail_btn = QPushButton("✉️ Mail Author")
        mail_btn.setProperty("class", "WebBtnSecondary")
        mail_btn.setEnabled(False)
        mail_btn.clicked.connect(lambda _, n=nid: self.mail_selected_article_author_for_nid(n))
        rh_row.addWidget(mail_btn)

        copy_btn = QPushButton("📋 Copy")
        copy_btn.setProperty("class", "WebBtnSecondary")
        copy_btn.setEnabled(False)
        copy_btn.clicked.connect(lambda _, n=nid: self.copy_article_text_for_nid(n))
        rh_row.addWidget(copy_btn)

        rc_layout.addLayout(rh_row)
        ...
        """)
        rc_layout.addWidget(body_edit, 1)

        v_splitter.addWidget(reader_card)
        v_splitter.setSizes([200, 280])

        layout.addWidget(v_splitter, 1)

        # Store references on tab_widget for fast access
        tab_widget.search_input = search_input
        tab_widget.count_badge = count_badge
        tab_widget.table = table
        tab_widget.subj_lbl = subj_lbl
        tab_widget.meta_lbl = meta_lbl
        tab_widget.body_edit = body_edit
        tab_widget.reply_btn = reply_btn
        tab_widget.mail_btn = mail_btn
        tab_widget.copy_btn = copy_btn
        tab_widget.selected_art = None

        return tab_widget

    def create_mailbox_tab(self):
        """Creates in-game mailbox tab."""
        tab_widget = QWidget()
        ...
        top_row = QHBoxLayout()
        hdr = QLabel("INBOX MESSAGES")
        hdr.setStyleSheet("font-size: 11px; font-weight: 800; color: #64748b;")
        top_row.addWidget(hdr, 1)

        self.mail_count_badge = QLabel("0 Messages")
        self.mail_count_badge.setStyleSheet("font-size: 11px; font-weight: 700; color: #94a3b8; background-color: #0f172a; border: 1px solid #1e293b; border-radius: 4px; padding: 4px 8px;")
        top_row.addWidget(self.mail_count_badge)
        left_layout.addLayout(top_row)

        self.mail_table = QTableWidget()
        self.mail_table.setColumnCount(4)
        self.mail_table.setHorizontalHeaderLabels(["From", "Date", "Subject", "Recipients"])
        self.mail_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.mail_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeToContents)
        self.mail_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.Stretch)
        self.mail_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeToContents)
        self.mail_table.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.mail_table.setSelectionMode(QAbstractItemView.SingleSelection)
        self.mail_table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        ...
        """)
        self.mail_table.itemSelectionChanged.connect(self.on_mail_row_selected)
        left_layout.addWidget(self.mail_table, 1)
        ...
        reader_card = QFrame()
        reader_card.setStyleSheet("background-color: #0f172a; border-radius: 6px; border: 1px solid #1e293b; padding: 12px;")
        rc_layout = QVBoxLayout(reader_card)
        rc_layout.setContentsMargins(8, 8, 8, 8)
        rc_layout.setSpacing(8)
        ...
        rh_row = QHBoxLayout()
        meta_v = QVBoxLayout()
        self.mail_subject_lbl = QLabel("Select a mail message to read")
        self.mail_subject_lbl.setStyleSheet("font-size: 14px; font-weight: 800; color: #f8fafc;")
        self.mail_meta_lbl = QLabel("--")
        self.mail_meta_lbl.setStyleSheet("font-size: 11px; color: #64748b;")
        meta_v.addWidget(self.mail_subject_lbl)
        meta_v.addWidget(self.mail_meta_lbl)
        rh_row.addLayout(meta_v, 1)

        self.mail_reply_btn = QPushButton("↩️ Reply")
        self.mail_reply_btn.setProperty("class", "WebBtnSecondary")
        self.mail_reply_btn.clicked.connect(self.reply_to_selected_mail)
        self.mail_reply_btn.setEnabled(False)
        rh_row.addWidget(self.mail_reply_btn)

        self.mail_delete_btn = QPushButton("🗑️ Delete")
        self.mail_delete_btn.setProperty("class", "WebBtnSecondary")
        self.mail_delete_btn.clicked.connect(self.delete_selected_mail)
        self.mail_delete_btn.setEnabled(False)
        rh_row.addWidget(self.mail_delete_btn)

        rc_layout.addLayout(rh_row)

        self.mail_body_viewer = QTextEdit()
        self.mail_body_viewer.setReadOnly(True)
        self.mail_body_viewer.setStyleSheet("""
            QTextEdit {
                background-color: #0b0f19;
                color: #e2e8f0;
                font-family: 'Segoe UI', system-ui, sans-serif;
        ...
        """)
        rc_layout.addWidget(self.mail_body_viewer, 1)

        right_layout.addWidget(reader_card, 1)
    ...
    def on_news_tab_changed(self, index):
        """Called when user switches tabs in the Newsgroup Reader."""
        if index < 0 or index >= self.news_tabs.count():
            return
        widget = self.news_tabs.widget(index)
        nid = widget.property("nid")
        if nid and nid != -1:
            self.selected_news_nid = nid
            self.load_newsgroup_articles_for_tab(nid)
        elif nid == -1:
            self.refresh_mailbox_ui()

    def update_news_globe_location_ui(self, room_name, globes=None):
        """Updates location banner and proximity check indicators when player changes rooms."""
        if not hasattr(self, 'news_room_loc_lbl'):
            return

        rid = getattr(self.news_engine, 'current_rid', None) or room_name
        self.news_room_loc_lbl.setText(f"📍 Location: {room_name}")

        if globes is None and hasattr(self, 'news_engine'):
            globes = self.news_engine.accessible_globes

        if globes:
            globe_desc = ", ".join([f"{g['name']} [NID {g['nid']}]" for g in globes])
            self.news_proximity_badge.setText(f"🟢 Active Globe in Room: {globe_desc}")
            self.news_proximity_badge.setStyleSheet("font-size: 11px; font-weight: 700; color: #34d399;")
            self.news_sync_btn.setEnabled(True)
        else:
            self.news_proximity_badge.setText("⚪ Outside Globe Room (Visit Town Inn or Adventurer Hall)")
            self.news_proximity_badge.setStyleSheet("font-size: 11px; font-weight: 700; color: #94a3b8;")

    def refresh_news_globe_ui(self):
        """Reloads articles, unread badges, and mailbox state."""
        if not hasattr(self, 'news_db'):
            return

        counts = self.news_db.get_globe_article_counts()
        total_arts = sum(counts.values())
        mail_items = self.news_db.get_mail_messages()
        total_mail = len(mail_items)

        if hasattr(self, 'news_db_count_lbl'):
            self.news_db_count_lbl.setText(f"DB: {total_arts} Articles | {total_mail} Mail")

        # Update tab titles with badges
        active_globes = getattr(self.news_engine, 'accessible_globes', [])
        active_nids = {g['nid'] for g in active_globes}

        for nid, tab_idx in getattr(self, 'newsgroup_tab_map', {}).items():
            ng_title = NEWSGROUP_NAMES.get(nid, "News").split(" ")[0]
            if nid == NID_ANNOUNCEMENTS: ng_title = "Designers' News"
            elif nid == NID_TOS_HALL: ng_title = "General News"

            unread = self.news_db.get_unread_count(nid)
            badge_str = f" 🔴 {unread}" if unread > 0 else ""
            focus_str = " 📍" if nid in active_nids else ""

            tab_label = f"📰 {ng_title}{badge_str}{focus_str}"
            self.news_tabs.setTabText(tab_idx, tab_label)

        # Refresh currently selected newsgroup tab
        if hasattr(self, 'selected_news_nid'):
            self.load_newsgroup_articles_for_tab(self.selected_news_nid)

        self.refresh_mailbox_ui()

    def load_newsgroup_articles_for_tab(self, nid):
        """Loads real articles into the dedicated newsgroup tab."""
        if nid not in getattr(self, 'newsgroup_widgets', {}):
            return

        tab_widget = self.newsgroup_widgets[nid]
        table = tab_widget.table
        articles = self.news_db.get_articles(nid, limit=250) if hasattr(self, 'news_db') else []

        unread_count = sum(1 for a in articles if not a.get('is_read'))
        tab_widget.count_badge.setText(f"{len(articles)} Posts ({unread_count} Unread)")

        table.setRowCount(len(articles))
        for row_idx, art in enumerate(articles):
            is_read = bool(art.get('is_read'))
            title_text = art.get('title', '(No Title)')
            is_reply = title_text.lower().startswith("re:") or title_text.lower().startswith("re ")

            num_item = QTableWidgetItem(f"#{art['article_num']}")
            num_item.setData(Qt.UserRole, art)
            num_item.setForeground(QColor("#38bdf8"))

            status_str = "✓ READ" if is_read else ("💬 REPLY" if is_reply else "🟡 UNREAD")
            status_item = QTableWidgetItem(status_str)
            status_color = QColor("#64748b") if is_read else (QColor("#a855f7") if is_reply else QColor("#f59e0b"))
            status_item.setForeground(status_color)

            date_val = art.get('timestamp', 0)
            date_str = datetime.fromtimestamp(date_val).strftime("%Y-%m-%d %H:%M") if date_val else "--"
            date_item = QTableWidgetItem(date_str)
            date_item.setForeground(QColor("#94a3b8"))

            author_item = QTableWidgetItem(art.get('poster', 'Unknown'))
            author_item.setForeground(QColor("#f8fafc"))
            ...
            table.setItem(row_idx, 2, date_item)
            table.setItem(row_idx, 3, author_item)
            table.setItem(row_idx, 4, title_item)

        if articles and table.currentRow() == -1:
            table.selectRow(0)

    def on_newsgroup_article_selected(self, nid):
        """Triggered when an article row is clicked in a newsgroup tab."""
        if nid not in getattr(self, 'newsgroup_widgets', {}):
            return
        tab_widget = self.newsgroup_widgets[nid]
        table = tab_widget.table
        ...
        tab_widget.selected_art = art
        art_num = art.get('article_num', 0)

        # Mark article read in DB
        self.news_db.mark_article_read(nid, art_num)

        date_val = art.get('timestamp', 0)
        date_str = datetime.fromtimestamp(date_val).strftime("%B %d, %Y at %I:%M %p") if date_val else "Unknown Date"
        poster = art.get('poster', 'Unknown')
        title = art.get('title', '(No Title)')
        group_name = NEWSGROUP_NAMES.get(nid, f"Newsgroup {nid}")

        tab_widget.subj_lbl.setText(title)
        tab_widget.meta_lbl.setText(f"Post #{art_num} by {poster} on {date_str} | {group_name}")

        body_text = art.get('body')
        if body_text:
            tab_widget.body_edit.setPlainText(body_text)
        else:
            tab_widget.body_edit.setHtml("""
                <div style='color: #64748b; font-style: italic; padding: 8px;'>
                    [Article body text downloading... Move to room with this globe to auto-fetch body via BP_REQ_ARTICLE]
                </div>
            """)

        tab_widget.reply_btn.setEnabled(True)
        tab_widget.mail_btn.setEnabled(True)
        tab_widget.copy_btn.setEnabled(bool(body_text))

    def filter_newsgroup_articles(self, nid, query):
        """Filters articles in the specified newsgroup tab by search query."""
        if nid not in getattr(self, 'newsgroup_widgets', {}):
            return
        tab_widget = self.newsgroup_widgets[nid]
        table = tab_widget.table
        ...
        for row in range(table.rowCount()):
            if not q:
                ...
                continue
            author = (table.item(row, 3).text() if table.item(row, 3) else "").lower()
            title = (table.item(row, 4).text() if table.item(row, 4) else "").lower()
    ...
    def reply_to_selected_article_for_nid(self, nid):
        if nid in getattr(self, 'newsgroup_widgets', {}):
            art = self.newsgroup_widgets[nid].selected_art
            if art:
                orig_title = art.get('title', '')
                reply_subj = orig_title if orig_title.lower().startswith("re:") else f"Re: {orig_title}"
                self.open_compose_article(default_nid=nid, reply_subject=reply_subj[:40])

    def mail_selected_article_author_for_nid(self, nid):
        if nid in getattr(self, 'newsgroup_widgets', {}):
            art = self.newsgroup_widgets[nid].selected_art
            if art:
                author = art.get('poster')
                orig_title = art.get('title', '')
                if author:
                    self.open_compose_mail(default_recipient=author, reply_subject=f"Re: {orig_title}"[:40])

    def copy_article_text_for_nid(self, nid):
        if nid in getattr(self, 'newsgroup_widgets', {}):
            art = self.newsgroup_widgets[nid].selected_art
            if art:
                title = art.get('title', '')
                author = art.get('poster', '')
                body = art.get('body', '')
                full_text = f"Subject: {title}\nAuthor: {author}\n\n{body}"
                clipboard = QGuiApplication.clipboard()
                clipboard.setText(full_text)
                self.show_toast_notification("📋 Article copied to clipboard.", category="info")

    def open_compose_article(self, default_nid=None, reply_subject=None):
        """Opens article composer dialog and posts article."""
        target_nid = default_nid or getattr(self, 'selected_news_nid', NID_TOS_HALL)
        dlg = M59ComposeArticleDialog(initial_nid=target_nid, default_subject=reply_subject or "", parent=self)
        if dlg.exec() == QDialog.Accepted:
            nid, subject, body = dlg.get_post_data()
            pkt = build_bp_post_article(nid, subject, body)
            self.inject_game_packet(pkt)

            char_author = self.char_name if getattr(self, 'char_name', None) and self.char_name != "--" else "LocalPlayer"
            sim_art_num = int(time.time()) % 1000000
            self.news_db.upsert_article_header(nid=nid, article_num=sim_art_num, timestamp=int(time.time()), poster=char_author, title=subject)
            self.news_db.update_article_body(nid, sim_art_num, body)

            self.show_toast_notification(f"📝 Posted article '{subject}' to {NEWSGROUP_NAMES.get(nid, 'News Globe')}.", category="info")
            self.refresh_news_globe_ui()

    def open_compose_mail(self, default_recipient=None, reply_subject=None):
        """Opens mail composer dialog and sends mail."""
        dlg = M59ComposeMailDialog(default_recipient=default_recipient or "", default_subject=reply_subject or "", parent=self)
        if dlg.exec() == QDialog.Accepted:
            recipient, subject, body = dlg.get_mail_data()
            pkt = build_bp_send_mail([0], recipient, subject, body)
            self.inject_game_packet(pkt)

            char_sender = self.char_name if getattr(self, 'char_name', None) and self.char_name != "--" else "LocalPlayer"
            sim_mail_idx = int(time.time()) % 1000000
            self.news_db.upsert_mail_message(mail_index=sim_mail_idx, sender=char_sender, recipients=recipient, msg_time=int(time.time()), subject=subject, body=body)

            self.show_toast_notification(f"✉️ Sent in-game mail to {recipient}.", category="info")
            self.refresh_mailbox_ui()

    def refresh_mailbox_ui(self):
        """Populates mailbox table with stored mail."""
        if not hasattr(self, 'mail_table'):
            return

        mail_items = self.news_db.get_mail_messages(limit=100) if hasattr(self, 'news_db') else []
        self.mail_count_badge.setText(f"{len(mail_items)} Messages")
        self.mail_table.setRowCount(len(mail_items))

        for row_idx, m in enumerate(mail_items):
            sender_item = QTableWidgetItem(m.get('sender', 'Unknown'))
            sender_item.setData(Qt.UserRole, m)
            sender_item.setForeground(QColor("#38bdf8"))
            ...
            subj_item = QTableWidgetItem(m.get('subject', '(No Subject)'))
            subj_item.setForeground(QColor("#f8fafc"))

            to_item = QTableWidgetItem(m.get('recipients', ''))
            to_item.setForeground(QColor("#cbd5e1"))

            self.mail_table.setItem(row_idx, 0, sender_item)
            self.mail_table.setItem(row_idx, 1, date_item)
            self.mail_table.setItem(row_idx, 2, subj_item)
            self.mail_table.setItem(row_idx, 3, to_item)

        if mail_items and self.mail_table.currentRow() == -1:
            self.mail_table.selectRow(0)

    def on_mail_row_selected(self):
        """Displays selected mail message in the reader pane."""
        selected_ranges = self.mail_table.selectedRanges()
        if not selected_ranges:
            return
        row = selected_ranges[0].topRow()
        ...
        self.selected_mail_data = m
        sender = m.get('sender', 'Unknown')
        recip = m.get('recipients', 'Me')
        subj = m.get('subject', '(No Subject)')
        ts = m.get('msg_time', 0)
        dt_str = datetime.fromtimestamp(ts).strftime("%B %d, %Y at %I:%M %p") if ts else "Unknown Date"

        self.mail_subject_lbl.setText(subj)
        self.mail_meta_lbl.setText(f"From: {sender} | To: {recip} | Sent: {dt_str}")
        self.mail_body_viewer.setPlainText(m.get('body', ''))

        self.mail_reply_btn.setEnabled(True)
        self.mail_delete_btn.setEnabled(True)

    def reply_to_selected_mail(self):
        if not getattr(self, 'selected_mail_data', None):
            return
        sender = self.selected_mail_data.get('sender', '')
        subj = self.selected_mail_data.get('subject', '')
        reply_subj = subj if subj.lower().startswith("re:") else f"Re: {subj}"
        self.open_compose_mail(default_recipient=sender, reply_subject=reply_subj[:40])

    def delete_selected_mail(self):
        if not getattr(self, 'selected_mail_data', None):
            return
        idx = self.selected_mail_data.get('mail_index')
        if idx is not None:
            self.news_db.delete_mail_message(idx)
            pkt = build_bp_delete_mail(idx)
            self.inject_game_packet(pkt)
            self.show_toast_notification(f"🗑️ Deleted mail message #{idx}.", category="info")
            self.refresh_mailbox_ui()

    def check_in_game_mail(self):
        pkt = build_bp_req_get_mail()
        self.inject_game_packet(pkt)
        self.show_toast_notification("📥 Requested in-game mail queue (BP_REQ_GET_MAIL)...", category="info")

    def inject_game_packet(self, packet_bytes):
        """Transmits raw binary packet to Meridian 59 client or logs for diagnostics."""
        if not packet_bytes:
            return
    ...
    def update_gps_room(self, room_name):
        ...
        if hasattr(self, 'update_news_globe_location_ui'):
            self.update_news_globe_location_ui(room_name)
