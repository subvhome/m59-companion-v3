...
class M59ComposeArticleDialog(QDialog):
    """
    Dialog to compose and post a new article to a Meridian 59 News Globe (BP_POST_ARTICLE).
    """
    def __init__(self, initial_nid=20, default_subject="", parent=None):
        super().__init__(parent)
        self.setWindowTitle("Compose News Globe Article")
        ...
        # Header Info
        header_lbl = QLabel("📝 Post to News Globe")
        header_lbl.setStyleSheet("font-size: 16px; font-weight: 800; color: #f8fafc;")
        ...
        self.nid_map = [
            (20, "General News (Adventurer Hall) - NID 20 (Public)"),
            (5, "Tales of Adventure (Bards) - NID 5 (Tales & Lore)"),
            (6, "Event Schedule - NID 6 (Tournaments & Events)"),
            (4, "Justicar News - NID 4 (Court & Trials)"),
            (9, "Designers' News - NID 9 (Admin Only)"),
            (3, "Game Updates - NID 3 (Technical)")
        ]
        for nid_val, name in self.nid_map:
            self.nid_combo.addItem(name, nid_val)

        # Select initial NID if provided
        for idx, (nid_val, _) in enumerate(self.nid_map):
            if nid_val == initial_nid:
                self.nid_combo.setCurrentIndex(idx)
                break

        form_layout.addRow("Target Globe:", self.nid_combo)

        # Subject Field
        subj_layout = QHBoxLayout()
        self.subject_input = QLineEdit()
        self.subject_input.setMaxLength(40)
        self.subject_input.setPlaceholderText("Article subject line (max 40 characters)...")
        if default_subject:
            self.subject_input.setText(default_subject)
        
        self.char_count_lbl = QLabel(f"{len(self.subject_input.text())}/40")
        self.char_count_lbl.setStyleSheet("color: #94a3b8; font-size: 11px; font-weight: 600;")
        self.subject_input.textChanged.connect(lambda txt: self.char_count_lbl.setText(f"{len(txt)}/40"))

        subj_layout.addWidget(self.subject_input, 1)
        subj_layout.addWidget(self.char_count_lbl)
        form_layout.addRow("Subject:", subj_layout)

        layout.addLayout(form_layout)

        # Body Field
        body_lbl = QLabel("Article Body:")
        body_lbl.setStyleSheet("font-size: 12px; font-weight: 700; color: #cbd5e1;")
        layout.addWidget(body_lbl)

        self.body_input = QTextEdit()
        self.body_input.setPlaceholderText("Write your article text here. Character name will be attached by the game server...")
        self.body_input.setStyleSheet("background-color: #0f172a; color: #f8fafc; border: 1px solid #334155; border-radius: 6px; padding: 8px; font-family: monospace; font-size: 12px;")
        ...
        self.cancel_btn.setProperty("class", "WebBtnSecondary")
        self.cancel_btn.clicked.connect(self.reject)
        btn_layout.addWidget(self.cancel_btn)

        self.post_btn = QPushButton("🚀 Post Article")
        self.post_btn.setProperty("class", "WebBtnPrimary")
        self.post_btn.clicked.connect(self.handle_post)
    ...
    def handle_post(self):
        ...
        if not subject:
            QMessageBox.warning(self, "Subject Required", "Please enter a subject line for your article (max 40 characters).")
            return
        if not body:
            QMessageBox.warning(self, "Body Required", "Please enter the body text for your article.")
            return
        self.accept()

    def get_post_data(self):
        nid = self.nid_combo.currentData()
        subject = self.subject_input.text().strip()
        body = self.body_input.toPlainText().strip()
        return nid, subject, body


class M59ComposeMailDialog(QDialog):
    """
    Dialog to compose and send in-game mail (BP_SEND_MAIL) to another player.
    """
    def __init__(self, default_recipient="", default_subject="", parent=None):
        super().__init__(parent)
        self.setWindowTitle("Compose In-Game Mail")
        ...
        # Header Info
        header_lbl = QLabel("✉️ Send In-Game Mail")
        header_lbl.setStyleSheet("font-size: 16px; font-weight: 800; color: #f8fafc;")
        layout.addWidget(header_lbl)
        ...
        self.recipient_input.setPlaceholderText("Recipient player name (e.g. Kran, Gwen, Balthazar)...")
        if default_recipient:
            self.recipient_input.setText(default_recipient)
        form_layout.addRow("To (Player):", self.recipient_input)
        ...
        # Body Field
        body_lbl = QLabel("Message Body:")
        body_lbl.setStyleSheet("font-size: 12px; font-weight: 700; color: #cbd5e1;")
        layout.addWidget(body_lbl)

        self.body_input = QTextEdit()
        self.body_input.setPlaceholderText("Type your mail message here...")
        self.body_input.setStyleSheet("background-color: #0f172a; color: #f8fafc; border: 1px solid #334155; border-radius: 6px; padding: 8px; font-family: monospace; font-size: 12px;")
        layout.addWidget(self.body_input, 1)
        ...
        self.send_btn = QPushButton("✉️ Send Mail")
        self.send_btn.setProperty("class", "WebBtnPrimary")
    ...
    def handle_send(self):
        ...
        body = self.body_input.toPlainText().strip()
        if not recipient:
            QMessageBox.warning(self, "Recipient Required", "Please enter the recipient character's name.")
            return
        if not subject:
            QMessageBox.warning(self, "Subject Required", "Please enter a subject line for your mail.")
            return
        if not body:
            QMessageBox.warning(self, "Body Required", "Please enter the message body.")
            return
        self.accept()
...
class M59FirstTimeSyncDialog(QDialog):
    """
    First-time initial sync confirmation dialog for news globes.
    Prompts: "[Newsgroup Name] has X messages available to sync. Proceed with full download?"
    """
    def __init__(self, newsgroup_name, nid, total_count, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Globe News — Initial Sync Confirmation")
        ...
        hdr = QLabel(f"🌐 Initial Sync: {newsgroup_name}")
        hdr.setStyleSheet("font-size: 15px; font-weight: 800; color: #38bdf8;")
        layout.addWidget(hdr)

        msg_text = (
            f"The newsgroup <b>{newsgroup_name}</b> (NID {nid}) has <b>{total_count}</b> "
            f"messages available to sync.<br><br>"
            f"Would you like to proceed with downloading the message archive now?<br><br>"
            f"<span style='color: #a855f7; font-size: 11px; font-weight: 700;'>"
            f"⚡ Note: This prompt is for the first time run only. Subsequent syncs for this globe "
            f"will run automatically and incrementally whenever you enter the room.</span>"
        )
        ...
        skip_btn.setProperty("class", "WebBtnSecondary")
        skip_btn.clicked.connect(self.reject)
        btn_row.addWidget(skip_btn)

        sync_btn = QPushButton(f"✓ Sync All ({total_count})")
        sync_btn.setProperty("class", "WebBtnPrimary")
